...
local function MoveSelection(self, offset, Songs)
    ...
    if offset ~= 0 then
                ...
                if type(Songs[pos]) ~= "string" then
                    ...
                    if Songs[pos][1]:GetDisplaySubTitle() ~= "" then
                        ...
                        self:GetChild("Wheel"):GetChild("Container" .. i)
                            :GetChild("Title"):zoom(.4):y(-10):maxwidth(650)
                    end
        ...
        for i = 1, 13 do
            ...
            self:GetChild("Wheel"):GetChild("Container" .. off)
                :GetChild("Title"):zoom(.6):y(-8):maxwidth(400)
    ...
    if offset ~= 0 then
        self:GetChild("BPMDummy"):GetChild("Num"):stoptweening():queuecommand(
            "BPMChange")
        self:GetChild("BPMDummy"):GetChild("Text"):stoptweening():queuecommand(
            "BPMChange")
        self:GetChild("BPM"):stoptweening():linear(.1):y(-155):sleep(0):y(-120)
        self:GetChild("BPMDummy"):stoptweening():linear(.1):y(-120):sleep(0):y(
            -95)
        self:GetChild("BPM"):GetChild("Num"):stoptweening():sleep(.1)
            :queuecommand("BPMChange")
        self:GetChild("BPM"):GetChild("Text"):stoptweening():sleep(.1)
            :queuecommand("BPMChange")
...
local function MoveDifficulty(self, offset, Songs)
    ...
    if offset == 0 then
        for _, v in pairs(GAMESTATE:GetHumanPlayers()) do
            ...
            if TF_WHEEL.DiffPos[v] > #Songs[TF_WHEEL.CurSong] - 1 then
                TF_WHEEL.DiffPos[v] = #Songs[TF_WHEEL.CurSong] - 1
            end
        end
    ...
    for i = 1, 2 do
        if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
            if GAMESTATE:IsSideJoined(((i == 1) and PLAYER_1 or PLAYER_2)) then
                self:GetChild("Diffs"):GetChild("Radar" .. i):GetChild(
                    "GrooveBG"):diffusealpha(1):playcommand("Move", {
                    .1, .5, true, i == 1, TF_WHEEL.DiffPos, Songs
                })
                self:GetChild("Diffs"):GetChild("Radar" .. i):GetChild(
                    "GrooveLine"):diffusealpha(1):playcommand("Move", {
                    .1, 1, false, i == 1, TF_WHEEL.DiffPos, Songs
                })
                self:GetChild("Diffs"):GetChild("DiffCon" .. i):visible(true)
                self:GetChild("Diffs"):GetChild("DiffCon" .. i):GetChild(
                    "DiffText"):settext(
                    DiffImage[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[(i ==
                        1) and PLAYER_1 or PLAYER_2] + 1]:GetDifficulty()]])
                self:GetChild("Diffs"):GetChild("DiffCon" .. i):GetChild(
                    "DiffBG"):diffuse(
                    DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[(i ==
                        1) and PLAYER_1 or PLAYER_2] + 1]:GetDifficulty()]])
            else
                self:GetChild("Diffs"):GetChild("DiffCon" .. i):visible(false)
                self:GetChild("Diffs"):GetChild("Radar" .. i):GetChild(
                    "GrooveBG"):diffusealpha(0)
                self:GetChild("Diffs"):GetChild("Radar" .. i):GetChild(
                    "GrooveLine"):diffusealpha(0)
            end
        else
            self:GetChild("Diffs"):GetChild("DiffCon" .. i):visible(false)
            self:GetChild("Diffs"):GetChild("Radar" .. i):GetChild("GrooveBG")
                :diffusealpha(0):playcommand("Move", {
                    .1, .5, true, i == 1, TF_WHEEL.DiffPos, Songs
                })
            self:GetChild("Diffs"):GetChild("Radar" .. i):GetChild("GrooveLine")
                :diffusealpha(0):playcommand("Move", {
                    .1, 1, false, i == 1, TF_WHEEL.DiffPos, Songs
                })
        end
...
return function(Style)
    ...
    for i = 1, 13 do
        ...
        Wheel[#Wheel + 1] = Def.ActorFrame {
            ...
            Def.Sprite {
                ...
                OnCommand = function(self)
                    self:zoom(.35):halign(0):x(-20):diffuse(DisplayColor[1] / 2,
                                                            DisplayColor[2] / 2,
                                                            DisplayColor[3] / 2,
                                                            DisplayColor[4])
                end
            ...
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    ...
                    self:zoom(.4):halign(0):y(-10):maxwidth(640):skewx(-.2)
                        :diffuse(DisplayColor[1], DisplayColor[2],
                                 DisplayColor[3], DisplayColor[4]):strokecolor(
                            DisplayColor[1] / 1.5, DisplayColor[2] / 1.5,
            ...
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    ...
                    -- Set size and colour.
                    self:zoom(.3):halign(0):maxwidth(650):skewx(-.2):diffuse(
                        DisplayColor[1], DisplayColor[2], DisplayColor[3],
                        DisplayColor[4]):strokecolor(DisplayColor[1] / 1.5,
                                                     DisplayColor[2] / 1.5,
                                                     DisplayColor[3] / 1.5,
                                                     DisplayColor[4])
                end
    ...
    local function MoveFunction(self, param)
        ...
        if type(param[6][TF_WHEEL.CurSong]) ~= "string" then
            local Val = {}
            colour = param[3] and
                         DiffColorsInside[TF_WHEEL.DiffTab[param[6][TF_WHEEL.CurSong][param[5][param[4] and
                             PLAYER_1 or PLAYER_2] + 1]:GetDifficulty()]] or
                         DiffColors[TF_WHEEL.DiffTab[param[6][TF_WHEEL.CurSong][param[5][param[4] and
                             PLAYER_1 or PLAYER_2] + 1]:GetDifficulty()]]
            zero = {{0, 0, 0}, colour}
            for i = 0, 4 do
                local temp = param[6][TF_WHEEL.CurSong][param[5][param[4] and
                                 PLAYER_1 or PLAYER_2] + 1]:GetRadarValues(
                                 param[4] and PLAYER_1 or PLAYER_2):GetValue(i)
                Val[#Val + 1] = (temp < 1) and temp or 1
            end

            if param[3] then
                self:linear(param[1]):SetVertices({
                    {{-80 * Val[2], -25 * Val[2], 0}, colour}, zero,
                    {{0, -85 * Val[1], 0}, colour},
                    {{0, -85 * Val[1], 0}, colour}, zero,
                    {{80 * Val[5], -25 * Val[5], 0}, colour},
                    {{80 * Val[5], -25 * Val[5], 0}, colour}, zero,
                    {{45 * Val[4], 70 * Val[4], 0}, colour},
                    {{45 * Val[4], 70 * Val[4], 0}, colour}, zero,
                    {{-45 * Val[3], 70 * Val[3], 0}, colour},
                    {{-45 * Val[3], 70 * Val[3], 0}, colour}, zero,
            ...
            self:linear(param[1]):SetVertices(empty)
        end
    end
    ...
    for i = 0, 1 do
        Diffs[#Diffs + 1] = Def.ActorFrame {
            Name = "DiffCon" .. i + 1,
            OnCommand = function(self)
                self:zoom(.35):xy(-280 + (i * 200), 10):visible(
                    GAMESTATE:IsSideJoined(((i == 0) and PLAYER_1 or PLAYER_2)))
            end,
            Def.Sprite {
                Texture = THEME:GetPathG("", "DDR/DiffMAX"),
                OnCommand = function(self)
                    self:diffuse(DisplayColor[1], DisplayColor[2],
                                 DisplayColor[3], 1):zoomx(i == 0 and 1 or -1)
                        :x(i * -120)
                end
            },
            Def.Quad {
                OnCommand = function(self)
                    self:zoomto(100, 100):xy(-60, 0):diffuse(0, 0, 0, 1)
                end
            },
            Def.Quad {
                Name = "DiffBG",
                OnCommand = function(self)
                    ...
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        self:diffuse(
                            DiffColors[TF_WHEEL.DiffTab[GroupsAndSongs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[i ==
                                0 and PLAYER_1 or PLAYER_2] + 1]:GetDifficulty()]])
                    end
                end
            },
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    ...
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        self:settext(
                            DiffImage[TF_WHEEL.DiffTab[GroupsAndSongs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[i ==
                                0 and PLAYER_1 or PLAYER_2] + 1]:GetDifficulty()]])
                    end
                end
            },
            Def.BitmapText {
                ...
                Text = (i + 1) .. "P",
                OnCommand = function(self)
                    self:zoomy(1.5):zoomx(2):xy(70 + (i * -260), -30):diffuse(
                        DisplayColor[1], DisplayColor[2], DisplayColor[3], 1)
                end
            }
        }
        ...
        Diffs[#Diffs + 1] = Def.ActorFrame {
            ...
            Def.ActorMultiVertex {
                ...
                InitCommand = function(self)
                    self:SetDrawState{Mode = "DrawMode_Triangles"}:xy(-200, 120)
                        :playcommand("Move", {
                            0, .5, true, i == 0, TF_WHEEL.DiffPos,
                            GroupsAndSongs
                        })
                end,
            ...
            Def.ActorMultiVertex {
                Name = "GrooveLine",
                InitCommand = function(self)
                    self:SetDrawState{Mode = "DrawMode_LineStrip"}:xy(-200, 120)
                        :SetLineWidth(4):playcommand("Move", {
                            0, 1, false, i == 0, TF_WHEEL.DiffPos,
                            GroupsAndSongs
                        })
                end,
                MoveCommand = MoveFunction
    ...
    local GRLocation = {{0, -105}, {110, -35}, {85, 80}, {-85, 80}, {-110, -35}}
    ...
    return Def.ActorFrame {
        ...
        Def.ActorFrame {
            ...
            PlayCurrentSongCommand = function(self)
                if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                    ...
                    TF_WHEEL.BG:LoadCachedBackground(THEME:GetPathG("Common",
                                                    "fallback background"))
                        :FullScreen()
                end
            end
            ...
            InitCommand = function(self)
                ...
                if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                    ...
                    if GroupsAndSongs[TF_WHEEL.CurSong][1]:HasBanner() then
                        ...
                        self:LoadCachedBanner(THEME:GetPathG("Common", "fallback banner"))
                    end
                else
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                self:zoom(.32):xy(-SCREEN_CENTER_X, -80):halign(0):diffuse(
                    DisplayColor[1], DisplayColor[2], DisplayColor[3],
                    DisplayColor[4])
            end
        },
        ...
        Def.ActorFrame {
            ...
            Def.Quad {
                OnCommand = function(self)
                    self:zoomto(160, 25):xy(-SCREEN_CENTER_X + 170, -95)
                        :MaskSource()
                end
            },
            Def.Quad {
                OnCommand = function(self)
                    self:zoomto(160, 36):xy(-SCREEN_CENTER_X + 170, -150)
                        :MaskSource()
                end
            }
        },
        ...
        Def.ActorFrame {
                ...
                BPMChangeCommand = function(self)
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        ...
                        if BPMS[1] ~= BPMS[2] then
                            self:diffuse(1, 0, 0, 1):settext(string.format(
                                                                 "%03.0f",
                                                                 BPMS[1]))
                                :queuecommand("BPMOne")
                        else
                            self:diffuse(1, 1, 0, 1):settext(string.format(
                                                                 "%03.0f",
                                                                 BPMS[1]))
                        end
                    else
                        self:diffuse(0, 1, 0, 1):settext("- - -")
                    end
                end,
                BPMOneCommand = function(self)
                    TF_WHEEL.CountingNumbers(self, self:GetText(),
                                             GroupsAndSongs[TF_WHEEL.CurSong][1]:GetDisplayBpms()[1],
                                             .4, "%03.0f")
            ...
            Def.BitmapText {
                ...
                Text = "bpm",
                OnCommand = function(self)
                    self:zoomy(.2):zoomx(.4):diffuse(1, 1, 0, 1):halign(0):xy(
                        58, 4):queuecommand("BPMChange")
                end,
                BPMChangeCommand = function(self)
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        local BPMS =
        ...
        Def.BitmapText {
            ...
            Font = "_noto sans 40px",
            OnCommand = function(self)
                self:diffuse(.8, .8, 1, 1):zoom(.5):xy(-SCREEN_CENTER_X + 40,
                                                       -120):halign(0)
            end
        },
        ...
        Def.ActorFrame {
            Name = "CDTitle",
            OnCommand = function(self)
                self:spin():effectmagnitude(0, -140, 0):xy(-SCREEN_CENTER_X +
                                                               250, -90):z(400)
            end,
            LoadCommand = function(self)
                for i = 1, self:GetNumChildren() do
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        -- It is, Get CDTitle.
                        self:GetChild("")[i]:visible(true):Load(
                            GroupsAndSongs[TF_WHEEL.CurSong][1]:GetCDTitlePath())
                            :zoom(TF_WHEEL.Resize(self:GetChild("")[i]:GetWidth(),
                                                  self:GetChild("")[i]:GetHeight(),
                                                  60, 60))
                    else
            ...
            Def.Sprite {
                InitCommand = function(self)
                    ...
                    self:zoom(TF_WHEEL.Resize(self:GetWidth(), self:GetHeight(),
                                              60, 60)):z(1):zbuffer(true)
                end
            }
        },

        -- Load the wheel.
        Wheel .. {OnCommand = function(self)
        ...
        Def.ActorFrame {
            ...
            Def.Sprite {
                Texture = THEME:GetPathG("", "DDR/MAXSelector"),
                OnCommand = function(self)
                    self:diffuseshift():effectcolor1(DisplayColor[1],
                                                     DisplayColor[2],
                                                     DisplayColor[3], 1)
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                self:zoom(.35):xy(-SCREEN_CENTER_X + 140, 120):diffuse(
                    DisplayColor[1], DisplayColor[2], DisplayColor[3], 1)
            end
        },

        Diffs .. {OnCommand = function(self)
