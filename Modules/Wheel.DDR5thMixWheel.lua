...
local function MoveSelection(self, offset, Songs)
    ...
    if offset ~= 0 then
        ...
        for i = 1, 13 do
            ...
            self:GetChild("Wheel"):GetChild("Container" .. i):linear(.1):x(
                transform):addy((offset * -45))
            ...
            if (i == IncOffset and offset == -1) or
                ...
                self:GetChild("Wheel"):GetChild("Container" .. i):sleep(0):addy(
                    (offset * -45) * -13)
                ...
                if type(Songs[pos]) ~= "string" then
                    ...
                    self:GetChild("Wheel"):GetChild("Container" .. i):GetChild(
                        "Title"):settext(Songs[pos][1]:GetDisplayMainTitle())
                else
                    -- It is not a song, Display group name instead.
                    self:GetChild("Wheel"):GetChild("Container" .. i):GetChild(
                        "Title"):settext(Songs[pos])
                end

                -- Set the width of the text.
                self:GetChild("Wheel"):GetChild("Container" .. i):GetChild(
                    "Title"):zoom(.7):y(-10):maxwidth(400)
                ...
                if type(Songs[pos]) ~= "string" then
                    ...
                    if Songs[pos][1]:GetDisplaySubTitle() ~= "" then
                        ...
                        self:GetChild("Wheel"):GetChild("Container" .. i)
                            :GetChild("Title"):zoom(.4):y(-12):maxwidth(650)
                    end

                    -- Set subtitle and artist to the values it has.
                    self:GetChild("Wheel"):GetChild("Container" .. i):GetChild(
                        "SubTitle"):settext(Songs[pos][1]:GetDisplaySubTitle())
                    self:GetChild("Wheel"):GetChild("Container" .. i):GetChild(
                        "Artist"):settext("/" ..
        ...
        if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
            ...
            if Songs[TF_WHEEL.CurSong][1]:HasBanner() then
                -- It does, Load it.
                self:GetChild("BannerUnderlay"):LoadCachedBanner(
                    Songs[TF_WHEEL.CurSong][1]:GetBannerPath())
            else
                -- It doesnt, Hide the banner.
                self:GetChild("BannerUnderlay"):LoadCachedBanner(THEME:GetPathG(
                                                                     "Common",
                                                                     "fallback banner"))
            end
        else
        ...
        self:GetChild("BannerUnderlay"):zoom(
            TF_WHEEL.Resize(self:GetChild("BannerUnderlay"):GetWidth(),
                            self:GetChild("BannerUnderlay"):GetHeight(), 224, 70))

        -- Load the top banner, This one shows when its done transitioning.
        self:GetChild("BannerOverlay"):diffusealpha(1):linear(.1)
            :diffusealpha(0):sleep(0):queuecommand("Load"):diffusealpha(1)

        -- Load the CDTitle command to do funky animation.
        self:GetChild("CDTitle"):linear(.05):zoomy(0):sleep(0):queuecommand(
            "Load")
        ...
        for i = 1, 13 do
            ...
            if i > 6 then pos = TF_WHEEL.CurSong + i - 13 end
            ...
            self:GetChild("Wheel"):GetChild("Container" .. off)
                :GetChild("Title"):zoom(.7):y(-10):maxwidth(400)
            ...
            if type(Songs[pos]) ~= "string" then
                ...
                self:GetChild("Wheel"):GetChild("Container" .. off):GetChild(
                    "SubTitle"):settext(Songs[pos][1]:GetDisplaySubTitle())
                self:GetChild("Wheel"):GetChild("Container" .. off):GetChild(
                    "Artist"):settext("/" .. Songs[pos][1]:GetDisplayArtist())
            else
                -- It is not a song so we set it to empty, Because groups dont have subtitles or atists.
                self:GetChild("Wheel"):GetChild("Container" .. off):GetChild(
                    "SubTitle"):settext("")
    ...
    if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
        ...
        self:GetChild("Dance"):SetAllStateDelays(14 /
                                                     Songs[TF_WHEEL.CurSong][1]:GetDisplayBpms()[2])
    end

    -- For every difficulty that we can display do.
    for i = 1, 6 do
        -- Hide Player1 diff.
        self:GetChild("Diffs"):GetChild("DiffName1P" .. i):GetChild("DiffBG")
            :diffusealpha(0)
        self:GetChild("Diffs"):GetChild("DiffName1P" .. i):GetChild("DiffText")
            :settext("")

        -- Hide player2 diff.
        self:GetChild("Diffs"):GetChild("DiffName2P" .. i):GetChild("DiffBG")
            :diffusealpha(0)
        self:GetChild("Diffs"):GetChild("DiffName2P" .. i):GetChild("DiffText")
            :settext("")

        -- Hide the feet difficulty meter.
        for i2 = 1, 9 do
            self:GetChild("Diffs"):GetChild("FeetCon" .. i):GetChild("Feet" ..
                                                                         i2)
                :diffusealpha(0)
        end
    end
    ...
    if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
        ...
        for i = 1, #Songs[TF_WHEEL.CurSong] - 1 do
            if i > 6 then break end

            -- Check if P1 is active.
            if GAMESTATE:IsSideJoined(PLAYER_1) then
                -- Diffuse the background of the difficulty selector.
                self:GetChild("Diffs"):GetChild("DiffName1P" .. i):GetChild(
                    "DiffBG"):diffuse(
                    DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i + 1]:GetDifficulty()]])

                -- Diffuse the text.
                self:GetChild("Diffs"):GetChild("DiffName1P" .. i):GetChild(
                    "DiffText"):settext(
                    DiffNames[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i + 1]:GetDifficulty()]])
            end

            -- Check if P2 is active.
            if GAMESTATE:IsSideJoined(PLAYER_2) then
                -- Diffuse the background of the difficulty selector.
                self:GetChild("Diffs"):GetChild("DiffName2P" .. i):GetChild(
                    "DiffBG"):diffuse(
                    DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i + 1]:GetDifficulty()]])
            ...
            -- Diffuse the feet.
            for i2 = 1, 9 do
                self:GetChild("Diffs"):GetChild("FeetCon" .. i):GetChild(
                    "Feet" .. i2):diffuse(
                    DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i + 1]:GetDifficulty()]])
                    :diffusealpha(.5)
            end
            ...
            for i2 = 1, Songs[TF_WHEEL.CurSong][i + 1]:GetMeter() do
                if i2 > 9 then break end
                -- Diffuse them.
                self:GetChild("Diffs"):GetChild("FeetCon" .. i):GetChild(
                    "Feet" .. i2):diffusealpha(1)
            end

            -- Extra check to diffuse the player difficulty selector on a 0 offset.
            if offset == 0 then
                self:GetChild("Diffs"):GetChild("DiffName1P" .. i):diffusealpha(
                    0)
                self:GetChild("Diffs"):GetChild("DiffName2P" .. i):diffusealpha(
                    0)
            end
            ...
            if TF_WHEEL.DiffPos[PLAYER_1] ~= i then
            ...
            if TF_WHEEL.DiffPos[PLAYER_1] > #Songs[TF_WHEEL.CurSong] - 1 then
                TF_WHEEL.DiffPos[PLAYER_1] = 1
            end

            -- Do effects on active position of player.
            if TF_WHEEL.DiffPos[PLAYER_1] == i then
                self:GetChild("Diffs"):GetChild("DiffName1P" .. i):bounce()
                    :effectmagnitude(-20, 0, 0):effectperiod(.6)
                self:GetChild("Diffs"):GetChild("DiffName1P" .. i):GetChild(
                    "DiffBG"):glowshift():effectperiod(1.2)
                self:GetChild("Diffs"):GetChild("DiffName1P" .. i):GetChild(
                    "DiffText"):glowshift():effectperiod(1.2)
            end

            -- Check the diffuse position of P2, if its not active, hide it.
            if TF_WHEEL.DiffPos[PLAYER_2] ~= i then
            ...
            -- Keep withing boundaries.
            if TF_WHEEL.DiffPos[PLAYER_2] > #Songs[TF_WHEEL.CurSong] - 1 then
                TF_WHEEL.DiffPos[PLAYER_2] = 1
            end

            -- Do effects on active position of player.
            if TF_WHEEL.DiffPos[PLAYER_2] == i then
                self:GetChild("Diffs"):GetChild("DiffName2P" .. i):bounce()
                    :effectmagnitude(20, 0, 0):effectperiod(.6)
                self:GetChild("Diffs"):GetChild("DiffName2P" .. i):GetChild(
                    "DiffBG"):glowshift():effectperiod(1.2)
                self:GetChild("Diffs"):GetChild("DiffName2P" .. i):GetChild(
    ...
    local Difficulties = #Songs[TF_WHEEL.CurSong] - 1
    ...
    if #Songs[TF_WHEEL.CurSong] - 1 > 3 then
    ...
    if offset ~= 0 then
        ...
        if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
            ...
            TF_WHEEL.CountingNumbers(self:GetChild("BPM"),
                                     self:GetChild("BPM"):GetText(),
                                     Songs[TF_WHEEL.CurSong][1]:GetDisplayBpms()[2],
                                     .1)
        else
            TF_WHEEL.CountingNumbers(self:GetChild("BPM"),
                                     self:GetChild("BPM"):GetText(), 0, .1)
        end
    end

    -- Check if offset is not 0.
    if offset ~= 0 then
        self:GetChild("Slider"):linear(.1):y(-176 +
                                                 (350 *
                                                     (TF_WHEEL.CurSong / #Songs)))
        ...
        self:GetChild("Slider"):y(-176 + (350 * (TF_WHEEL.CurSong / #Songs)))
    end
end
...
return function(Style)
    ...
    for i = 1, 13 do
        ...
        Wheel[#Wheel + 1] = Def.ActorFrame {
            ...
            OnCommand = function(self)
                self:xy((offset * offset) * 3, offset * 45)
            end,
            ...
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    ...
                    self:zoom(.4):halign(0):y(-12):maxwidth(640):diffuse(
                        DisplayColor[1], DisplayColor[2], DisplayColor[3],
                        DisplayColor[4]):skewx(-.2)
            ...
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    ...
                    -- Set size and colour.
                    self:zoom(.3):halign(0):maxwidth(650):diffuse(
                        DisplayColor[1], DisplayColor[2], DisplayColor[3],
                        DisplayColor[4]):skewx(-.2)
                end
            },
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    ...
                    -- Set size and colour.
                    self:zoom(.3):halign(0):y(10):maxwidth(650):diffuse(
                        DisplayColor[1], DisplayColor[2], DisplayColor[3],
                        DisplayColor[4]):skewx(-.2)
                end
            }
    ...
    for i = 1, 6 do
        ...
        Diffs[#Diffs + 1] = Def.ActorFrame {
            ...
            Def.BitmapText {
                ...
                InitCommand = function(self)
                    ...
                    self:zoom(.18):diffuse(0, 0, 0, 1):xy(-17, 22 * i):maxwidth(
                        140)
                end
            }
        }
        ...
        Diffs[#Diffs + 1] = Def.ActorFrame {
            ...
            Def.BitmapText {
                ...
                InitCommand = function(self)
                    -- Set size, colour, position and maxwidth.
                    self:zoom(.18):diffuse(0, 0, 0, 1):xy(154, 22 * i):maxwidth(
                        140)
                end
            }
        }
    ...
    return Def.ActorFrame {
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                self:zoom(.45):xy(-(SCREEN_CENTER_X * OffsetMath), -60)
                    :halign(0):diffuse(DisplayColor[1], DisplayColor[2],
                                       DisplayColor[3], DisplayColor[4])
            end
        },
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 180, -162):zoom(.05)
                    :diffuse(DisplayColor[1] / 1.5, DisplayColor[2] / 1.5,
                             DisplayColor[3] / 1.5, DisplayColor[4])
            end
        },
        ...
        Def.BitmapText {
            ...
            Text = "BPM",
            OnCommand = function(self)
                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 178, -162):zoom(.2)
                    :zoomx(.3):diffuse(DisplayColor[1], DisplayColor[2],
                                       DisplayColor[3], DisplayColor[4])
            end
        },
        ...
        Def.BitmapText {
            ...
            OnCommand = function(self)
                ...
                if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                    ...
                    self:settext(string.format("%.0f",
                                               GroupsAndSongs[TF_WHEEL.CurSong][1]:GetDisplayBpms()[2]))
                end

                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 190, -134):zoomy(.6)
                    :diffusebottomedge(1, .5, 0, 1):diffusetopedge(1, 1, 0, 1)
                    :skewx(-.2)
            end
        },
        ...
        Def.Sprite {
            Texture = THEME:GetPathG("", "DDR/DiffSel"),
            OnCommand = function(self)
                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 70, -154):zoom(.05)
                    :zoomx(.07):diffuse(DisplayColor[1] / 1.5,
                                        DisplayColor[2] / 1.5,
                                        DisplayColor[3] / 1.5, DisplayColor[4])
            end
        },
        ...
        Def.BitmapText {
            ...
            Text = "STAGE",
            OnCommand = function(self)
                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 68, -154):zoom(.2)
                    :zoomx(.3):diffuse(DisplayColor[1], DisplayColor[2],
                                       DisplayColor[3], DisplayColor[4])
            end
        },
        ...
        Def.BitmapText {
            ...
            Font = "_noto sans 40px",
            OnCommand = function(self)
                self:diffuse(0, .5, 0, 1):strokecolor(0, .5, 0, 1):zoom(.3):xy(
                    -(SCREEN_CENTER_X * OffsetMath) + 68, -136):skewx(-.2)
            end
        },
        ...
        Def.Sprite {
            ...
            InitCommand = function(self)
                self:zoom(TF_WHEEL.Resize(self:GetWidth(), self:GetHeight(),
                                          224, 70)):xy(
                    -(SCREEN_CENTER_X * OffsetMath) + 150, -84)
            end
        },
        ...
        Def.Sprite {
            ...
            InitCommand = function(self)
                ...
                self:zoom(TF_WHEEL.Resize(self:GetWidth(), self:GetHeight(), 50,
                                          50)):xy(
                    -(SCREEN_CENTER_X * OffsetMath) + 240, -102)
            end,
            LoadCommand = function(self)
                -- Check if its a song.
                if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                ...
                self:zoom(TF_WHEEL.Resize(self:GetWidth(), self:GetHeight(), 60,
                                          60)):zoomy(0):linear(.05):zoom(
                    TF_WHEEL.Resize(self:GetWidth(), self:GetHeight(), 60, 60))
            end
        },
        ...
        Def.Sprite {
            Texture = THEME:GetPathG("", "DDR/DiffSel"),
            OnCommand = function(self)
                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 120, -42):zoom(.033)
                    :zoomx(-.08):diffuse(DisplayColor[1] / 1.5,
                                         DisplayColor[2] / 1.5,
                                         DisplayColor[3] / 1.5, DisplayColor[4])
            end
        },
        ...
        Def.Sprite {
            Texture = THEME:GetPathG("", "DDR/DiffSel"),
            OnCommand = function(self)
                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 174, -42):zoom(.033)
                    :zoomx(.08):diffuse(DisplayColor[1] / 1.5,
                                        DisplayColor[2] / 1.5,
                                        DisplayColor[3] / 1.5, DisplayColor[4])
            end
        ...
        Def.BitmapText {
            ...
            Font = "_noto sans 40px",
            OnCommand = function(self)
                self:diffuse(DisplayColor[1] / 4, DisplayColor[2] / 4,
                             DisplayColor[3] / 4, DisplayColor[4]):strokecolor(
                    DisplayColor[1] / 4, DisplayColor[2] / 4,
                    DisplayColor[3] / 4, DisplayColor[4]):xy(
                    -(SCREEN_CENTER_X * OffsetMath) + 36, -40):zoomy(.23):zoomx(
                    .33):skewx(-.25)
            end
        },
        ...
        Def.BitmapText {
            ...
            OnCommand = function(self)
                self:diffuse(DisplayColor[1] / 4, DisplayColor[2] / 4,
                    ...
                    DisplayColor[1] / 4, DisplayColor[2] / 4,
                    DisplayColor[3] / 4, DisplayColor[4]):xy(
                    -(SCREEN_CENTER_X * OffsetMath) + 260, -40):zoomy(.23)
                    :zoomx(.33):skewx(-.25)
            end
        },
        ...
        Def.BitmapText {
            ...
            Text = "DIFFICULTY",
            OnCommand = function(self)
                self:xy(-(SCREEN_CENTER_X * OffsetMath) + 148, -42):zoom(.18)
                    :zoomx(.3):diffuse(DisplayColor[1], DisplayColor[2],
                                       DisplayColor[3], DisplayColor[4])
            end
        },
        ...
        Diffs .. {
            OnCommand = function(self)
                self:x(-(SCREEN_CENTER_X * OffsetMath) + 78):valign(0)
            end
        },
        ...
        Wheel ..
            {
                OnCommand = function(self)
                    self:x((SCREEN_CENTER_X * OffsetMath) - 320)
                end
            },
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                self:zoom(.65):xy((SCREEN_CENTER_X * OffsetMath) - 180, -2)
                    :faderight(1):diffuseshift():effectcolor1(1, 1, 1, .9)
                    :effectcolor2(DisplayColor[1], DisplayColor[2],
                                  DisplayColor[3], .5)
            end
        },
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                self:zoom(.35):diffuse(.8, .8, 0, 1):x((SCREEN_CENTER_X *
                                                           OffsetMath) - 20)
            end
        },
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                self:zoom(.35):diffuse(1, 0, 0, 1):xy((SCREEN_CENTER_X *
                                                          OffsetMath) - 20,
                                                      -176 +
                                                          (350 *
                                                              (TF_WHEEL.CurSong /
