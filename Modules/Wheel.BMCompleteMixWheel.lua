...
local function MoveSelection(self, offset, Songs)
    ...
    if offset ~= 0 then
        ...
        for i = 1, 5 do
            ...
            self:GetChild("LPCon"):GetChild("LP" .. i):linear(.1):addx((offset *
                                                                           400))
            self:GetChild("LPCon"):GetChild("LP" .. i):GetChild("Container")
                :sleep(0):y(320)
            ...
            if (i == IncOffset and offset == -1) or
                ...
                self:GetChild("LPCon"):GetChild("LP" .. i):sleep(0):addx(
                    (offset * 400) * -5)

                -- Check if it's a song.
                if type(Songs[pos]) ~= "string" then
                    if Songs[pos][1]:HasBanner() then
                        self:GetChild("LPCon"):GetChild("LP" .. i):GetChild(
                            "Container"):GetChild("LPPicture"):LoadCachedBanner(
                ...
                self:GetChild("LPCon"):GetChild("LP" .. i):GetChild("Container")
                    :GetChild("LPPicture"):setsize(180, 180)
            end
        end
        ...
        for i = 1, 5 do
            ...
            if i > 2 then pos = TF_WHEEL.CurSong + i - 5 end
            ...
            if type(Songs[pos]) ~= "string" then
                if Songs[pos][1]:HasBanner() then
                    self:GetChild("LPCon"):GetChild("LP" .. off):GetChild(
                        "Container"):GetChild("LPPicture"):LoadCachedBanner(
                        Songs[pos][1]:GetBannerPath())
                else
                    self:GetChild("LPCon"):GetChild("LP" .. off):GetChild(
            ...
            self:GetChild("LPCon"):GetChild("LP" .. off):GetChild("Container")
                :GetChild("LPPicture"):setsize(180, 180)
        end
    end
    ...
    if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
        ...
        if not Songs[TF_WHEEL.CurSong][1]:HasBanner() then
            ...
            self:GetChild("BannerAFT"):GetChild("BannerText"):settext(
                Songs[TF_WHEEL.CurSong][1]:GetDisplayMainTitle()):maxwidth(280)
                :maxheight(120):Regen()
        else
            self:GetChild("Banner"):LoadCachedBanner(
            ...
            self:GetChild("BannerAFT"):GetChild("BannerText"):settext("")
                :maxwidth(280):maxheight(120):Regen()
        end

        local Genre = Songs[TF_WHEEL.CurSong][1]:GetGenre()

        if Genre == "" then Genre = "UNKNOWN" end

        self:GetChild("GenreAFT"):GetChild("GenreText"):settext("\"" ..
                                                                    ToLower(
                                                                        Genre) ..
                                                                    "\"")
            :maxwidth(280):maxheight(60):Regen()
        ...
        if SONGMAN:GetSongGroupBannerPath(Songs[TF_WHEEL.CurSong]) ~= "" then
            ...
            -- Set name to group.
            self:GetChild("BannerAFT"):GetChild("BannerText"):settext(
                Songs[TF_WHEEL.CurSong]):maxwidth(280):maxheight(120):Regen()
        end

        self:GetChild("GenreAFT"):GetChild("GenreText"):settext("\"pack\"")
            :maxwidth(280):maxheight(60):Regen()
    end
...
local function MoveDifficulty(self, offset, Songs)
    ...
    if type(Songs[TF_WHEEL.CurSong]) == "string" then
        ...
        for i = 1, 6 do
            self:GetChild("Diffs"):GetChild("Star" .. i):diffuse(1, 0, 0, 1)
                :diffusealpha(0)
        end

        -- We get the Meter from the game, And make it so it stays between 6 which is the Max Stars we support.
        local DiffCount = math.floor(
                              Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] +
                                  1]:GetMeter() / 2)
...
return function(Style)
    ...
    for i = 1, 5 do
        ...
        LPs[#LPs + 1] = Def.ActorFrame {
            ...
            Def.ActorFrame {
                ...
                OnCommand = function(self)
                    self:y(320):zoom(.8)
                    if i == 3 then self:y(340) end
                end,
                Def.ActorProxy {
                    Name = "LPBG",
                    InitCommand = function(self)
                        self:SetTarget(self:ForParent(4):GetChild("LPBGCon")
                                           :GetChild("LPBG")):zoom(.9)
                    end
                },
                Def.Sprite {
                    Name = "LPMask",
                ...
                Def.Sprite {
                    ...
                    OnCommand = function(self)
                        ...
                        self:setsize(180, 180):MaskDest():ztestmode(
    ...
    for i = 1, 2 do
        Sel[#Sel + 1] = Def.ActorFrame {
            ...
            OnCommand = function(self)
                self:x(-150)
                if i == 2 then self:zoomx(-1):x(150) end
            end,
            EnableEffectCommand = function(self)
                self:GetChild("Inside"):diffuseshift():effectcolor1(color(
                                                                        "#ec0393"))
                    :effectcolor2(color("#b1e425")):effectperiod(.2)
                self:GetChild("Outside"):diffuseshift():effectcolor1(color(
                                                                         "#062ebf"))
                    :effectcolor2(color("#ff5e03")):effectperiod(.2)
    ...
    for i = 1, 6 do
        Diff[#Diff + 1] = Def.ActorProxy {
            Name = "StarBG" .. i,
            InitCommand = function(self)
                self:SetTarget(self:ForParent(2):GetChild("BGStar")):x(35 *
                                                                           (i -
                                                                               4.5))
    ...
    return Def.ActorFrame {
        ...
        MenuLeftCommand = function(self)
            self:GetChild("Arrows"):GetChild("Arrow1"):queuecommand(
                "EnableEffect")
        ...
        MenuRightCommand = function(self)
            self:GetChild("Arrows"):GetChild("Arrow2"):queuecommand(
                "EnableEffect")
            MoveSelection(self, 1, GroupsAndSongs)
            MoveDifficulty(self, 0, GroupsAndSongs)
        end,
        ...
        Def.Sprite {
            ...
            OnCommand = function(self) self:zoom(0.75):y(-80) end
        },
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                ...
                self:y(-70):zoom(TF_WHEEL.Resize(self:GetWidth(),
                                                 self:GetHeight(),
                                                 (512 / 12) * 5, (160 / 12) * 5))
            end
        },
        ...
        Def.ActorFrameTexture {
            Name = "BannerAFT",
            InitCommand = function(self)
                self:SetTextureName("BannerAFT"):SetWidth(280):SetHeight(120)
            ...
            Def.Text {
                ...
                OnCommand = function(self)
                    ...
                    self:maxwidth(280):maxheight(120):Regen()
        ...
        Def.Sprite {
            Texture = "BannerAFT",
            OnCommand = function(self)
                self:y(-50):croptop(.32):diffuse(1, .5, 0, 1):diffusebottomedge(
                    1, 1, 0, 1)
            end
        },
        ...
        Def.ActorFrameTexture {
            Name = "GenreAFT",
            InitCommand = function(self)
                self:SetTextureName("GenreAFT"):SetWidth(280):SetHeight(60)
                    :EnableAlphaBuffer(true):Create():Draw()
            end,
            -- Global Centered Banner Text, Incase there is no banner.
            Def.Text {
                ...
                OnCommand = function(self)
                    ...
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) == "string" then
                        -- Check if group has banner, If so, Set text to empty
                        self:settext("\"pack\""):maxwidth(280):maxheight(60)
                            :Regen()
                        -- not group.
                    else
                        local Genre =
                            GroupsAndSongs[TF_WHEEL.CurSong][1]:GetGenre()
                        if Genre == "" then
                            Genre = "UNKNOWN"
                        end

                        -- Check if we have banner, if not, set text to song title.
                        self:settext("\"" .. ToLower(Genre) .. "\""):maxwidth(
                            280):maxheight(60):Regen()
                    end
        ...
        Def.Sprite {
            Texture = "GenreAFT",
            OnCommand = function(self)
                self:y(-105):croptop(.32):diffuse(1, .5, 0, 1)
                    :diffusebottomedge(1, 1, 0, 1)
            end
        },
        ...
        Def.ActorFrame {
            ...
            Def.Sprite {
                Texture = THEME:GetPathG("", "StarSharp.png"),
                InitCommand = function(self)
                    self:zoom(.02):MaskSource(true)
                end
            },
            Def.Sprite {
                Texture = THEME:GetPathG("", "StarSharp.png"),
                InitCommand = function(self)
                    self:zoom(.05):MaskDest()
                end
            },
            Def.Sprite {
                Texture = THEME:GetPathG("", "StarSharp.png"),
                InitCommand = function(self)
                    self:zoom(.04):diffuse(0, 0, 0, 1):MaskDest()
                end
            },
            Def.Sprite {
                Texture = THEME:GetPathG("", "StarSharp.png"),
                InitCommand = function(self)
                    self:zoom(.03):MaskDest()
                end
            }
        },
