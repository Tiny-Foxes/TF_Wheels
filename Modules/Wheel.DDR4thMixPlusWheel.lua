    ...
    for i = 1, 7 do
        ...
        if type(Songs[pos]) ~= "string" then
            ...
            if SONGMAN:GetSongGroupBannerPath(Songs[pos]) ~= "" then
                -- For every banner on current row load the next banner.
                self:GetChild("Banners"):GetChild(CurRow .. i):GetChild(
                    "BannerCon"):GetChild("Banner"):diffusealpha(1)
                    :LoadCachedBanner(SONGMAN:GetSongGroupBannerPath(Songs[pos]))
            else
                self:GetChild("Banners"):GetChild(CurRow .. i):GetChild(
                    "BannerCon"):GetChild("Banner"):diffusealpha(0)
            end
        end

        -- Do the zoom on the banners, We set them to w128 h40.
        self:GetChild("Banners"):GetChild(CurRow .. i):GetChild("BannerCon")
            :GetChild("Banner"):zoom(TF_WHEEL.Resize(
                                         self:GetChild("Banners"):GetChild(
                                             CurRow .. i):GetChild("BannerCon")
                                             :GetChild("Banner"):GetWidth(),
                                         self:GetChild("Banners")
                                             :GetChild(CurRow .. i)
                                             :GetChild("BannerCon"):GetChild(
                                                 "Banner"):GetHeight(), 128, 40))
        ...
        if type(Songs[pos]) ~= "string" then
            ...
            if SONGMAN:GetSongGroupBannerPath(Songs[pos]) ~= "" then
                ...
                self:GetChild("Banners"):GetChild(CurRow .. i):GetChild(
                    "BannerCon"):GetChild("BannerText"):diffusealpha(1):zoom(.5)
                self:GetChild("Banners"):GetChild(CurRow .. i):GetChild(
                    "BannerCon"):GetChild("BannerText"):settext(Songs[pos])
            end
        end

        -- Grab the offscreen current row, And move it to the screen
        self:GetChild("Banners"):GetChild(CurRow .. i):GetChild("BannerCon")
            :sleep(sleep / 8):linear(.5):x(0)

        -- Wait for a few sec till the new row is on screen, And then make it move away.
        self:GetChild("Banners"):GetChild(OldRow .. i):GetChild("BannerCon")
            :sleep((sleep / 8) + .4):x(1280)

        -- Grab the slider thats above the wheel and move it ofscreen.
        self:GetChild("SliderCon"):GetChild("Slider" .. i):linear(0.2):x((1280 *
                                                                             (offset *
                                                                                 -1)))
            :sleep(0.00001):diffusealpha(0):x((1280 * offset)):diffusealpha(1)

        -- Check if its a song.
        if type(Songs[pos]) ~= "string" then
            if Songs[pos][1]:HasBanner() then
                -- Make the slider load the current row banners.
                self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                    "Banner"):diffusealpha(1):LoadCachedBanner(
                    Songs[pos][1]:GetBannerPath())
            else
                self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                    "Banner"):LoadCachedBanner(
        ...
        self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild("Banner")
            :zoom(TF_WHEEL.Resize(self:GetChild("SliderCon")
                                      :GetChild("Slider" .. i)
                                      :GetChild("Banner"):GetWidth(),
                                  self:GetChild("SliderCon")
                                      :GetChild("Slider" .. i)
                                      :GetChild("Banner"):GetHeight(), 256, 80))

        -- Get the amount of songs text that displays the current song we're on.
        self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild("CurSong")
            :settext(pos .. "/" .. #Songs)
        ...
        for i3 = 1, 6 do
            ...
            self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                "DiffCon"):GetChild("DiffDisplay" .. i3):GetChild("Feet")
                :diffusealpha(0)

            -- do the same for all the lvl text.
            self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                "DiffCon"):GetChild("DiffDisplay" .. i3):GetChild("level")
                :diffusealpha(0)
            ...
            if #Songs[pos] > i3 and type(Songs[pos]) ~= "string" then
                ...
                self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                    "DiffCon"):GetChild("DiffDisplay" .. i3):GetChild("Feet")
                    :diffuse(DiffColors[TF_WHEEL.DiffTab[Songs[pos][i3 + 1]:GetDifficulty()]])

                -- Color the level display to the difficulties.
                self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                    "DiffCon"):GetChild("DiffDisplay" .. i3):GetChild("level")
                    :diffuse(DiffColors[TF_WHEEL.DiffTab[Songs[pos][i3 + 1]:GetDifficulty()]])
                    :settext(Songs[pos][i3 + 1]:GetMeter())
            end
        end
    end
...
local function MoveSelection(self, offset, Songs)
    ...
    for i = 1, 7 do
        self:GetChild("SliderCon"):GetChild("Slider" .. i):linear(.1):x((256 *
                                                                            ((i -
                                                                                3) +
                                                                                (SongPos *
                                                                                    -1))))
    ...
    self:GetChild("Banners"):GetChild(CurRow .. SongPos + 3):GetChild(
        "BannerCon"):linear(.1):x(32):effectclock("Beat"):glowshift()
    ...
    if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
        ...
        self:GetChild("Subtitle"):settext(
            Songs[TF_WHEEL.CurSong][1]:GetDisplaySubTitle())

        if Songs[TF_WHEEL.CurSong][1]:HasBanner() then
            self:GetChild("Banners"):GetChild(CurRow .. SongPos + 3):GetChild(
                "BannerCon"):GetChild("Banner"):position(0)
        end
    else
        -- Change the current song title.
        self:GetChild("Title"):settext(Songs[TF_WHEEL.CurSong])
...
local function UpdateSelection(self, Songs)
    ...
    for i = 1, 7 do
        ...
        -- Grab the slider thats above the wheel and move it ofscreen.
        self:GetChild("SliderCon"):GetChild("Slider" .. i):linear(.1)
            :diffusealpha(0):sleep((7 / 8) + .3)
            :x((256 * ((i - 3) + (-2 * -1)))):linear(.2):diffusealpha(1)
        ...
        if type(Songs[pos]) ~= "string" then
            if Songs[pos][1]:HasBanner() then
                -- For every banner on current row load the next banner.
                self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                    "Banner"):LoadCachedBanner(Songs[pos][1]:GetBannerPath())
            else
                self:GetChild("SliderCon"):GetChild("Slider" .. i):GetChild(
                    "Banner"):LoadCachedBanner(
                    THEME:GetPathG("Common", "fallback banner"))
...
local function StartSelection(self, Songs)
    ...
    for i = SongPos + 3, 7 do
        -- Let them fly off.
        self:GetChild("Banners"):GetChild(CurRow .. i):GetChild("BannerCon")
            :sleep(offset / 8):linear(.5):x(-1280)
        offset = offset + 1
    end
    ...
    for i = 1, #Songs[TF_WHEEL.CurSong] - 1 do
        ...
        for i2 = 1, 9 do
            self:GetChild("Diffs"):GetChild("Feet" .. i .. i2):sleep(.5):linear(
                .5):diffuse(
        ...
        if GAMESTATE:IsSideJoined(PLAYER_1) then
            ...
            self:GetChild("Diffs"):GetChild("DiffSelector" .. i .. "1")
                :GetChild("DiffCon"):sleep(.5):linear(.5):diffuse(
                    DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i + 1]:GetDifficulty()]])

            -- Set the difficulty name on the left side of the selector.
            self:GetChild("Diffs"):GetChild("DiffSelector" .. i .. "1")
                :GetChild("DiffName"):sleep(.5):linear(.5):diffuse(
                    DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i + 1]:GetDifficulty()]])
                :settext(DiffNames[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i +
                             1]:GetDifficulty()]])
        end
        ...
        if GAMESTATE:IsSideJoined(PLAYER_2) then
            ...
            self:GetChild("Diffs"):GetChild("DiffSelector" .. i .. "2")
                :GetChild("DiffCon"):sleep(.5):linear(.5):diffuse(
                    DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][i + 1]:GetDifficulty()]])
...
local function MoveDifficulty(self, offset, Songs)
    ...
    if GAMESTATE:IsSideJoined(self.pn) then
        ...
        for i = 1, 6 do
            self:GetChild("Diffs"):GetChild("DiffSelector" .. i .. pn)
                :stopeffect()
        end
        ...
        self:GetChild("Diffs"):GetChild("DiffSelector" ..
                                            TF_WHEEL.DiffPos[self.pn] .. pn)
            :effectclock("Beat"):glowshift()
    end
end
...
return function(Style)
    ...
    for i = 1, 7 do
        ...
        local pos = TF_WHEEL.CurSong + i - 4
                    ...
                    OnCommand = function(self)
                        ...
                        if type(GroupsAndSongs[pos]) ~= "string" then
                            ...
                            if #GroupsAndSongs[pos] > i2 then
                                ...
                                self:diffuse(
                                    DiffColors[TF_WHEEL.DiffTab[GroupsAndSongs[pos][i2 +
                                        1]:GetDifficulty()]])
                            end
                        end
                    end
                        ...
                        if type(GroupsAndSongs[pos]) ~= "string" then
                            ...
                            if #GroupsAndSongs[pos] > i2 then
                                ...
                                self:diffuse(
                                    ...
                                    GroupsAndSongs[pos][i2 + 1]:GetMeter())
                            end
                        end
                    end
                }
        ...
        Slider[#Slider + 1] = Def.ActorFrame {
            Name = "Slider" .. i,
            OnCommand = function(self)
                self:xy((256 * (i - 4)), -56):diffusealpha(0):linear(.5)
                    :diffusealpha(1)
            end,
        ...
        for i2 = 1, 2 do
            Banners[#Banners + 1] = Def.ActorFrame {
                ...
                InitCommand = function(self)
                    self:rotationz(-45):xy((64 * (i - 4)), 80)
                end,
    ...
    for i = 1, 6 do
        ...
        for i2 = 1, 2 do
            Diffs[#Diffs + 1] = Def.ActorFrame {
                ...
                Def.Sprite {
                    ...
                    OnCommand = function(self)
                        self:zoom(.5):y((i * 32)):x(((i2 - 1.5) * 224))
                            :diffusealpha(0)
                    end
                },
                ...
                Def.BitmapText {
                    ...
                    Text = "Practice",
                    OnCommand = function(self)
                        self:maxwidth(60):zoom(.5):zoomy(.25):y((i * 32)):x(
                            ((i2 - 1.5) * 224)):diffusealpha(0)
                    end
                }
            }
        end
        ...
        for i2 = 1, 9 do
            Diffs[#Diffs + 1] = Def.Sprite {
                ...
                Texture = THEME:GetPathG("", "DDR/Feet.png"),
                OnCommand = function(self)
                    self:zoom(.125):y((i * 32)):x(((i2 - 5) * 16)):diffusealpha(
                        0)
                end
            }
        end
    ...
    return Def.ActorFrame {
        ...
        StartCommand = function(self)
            ...
            if DiffSelection then
                ...
                if StartOptions then
                    SCREENMAN:GetTopScreen():SetNextScreenName(
                        "ScreenPlayerOptions"):StartTransitioningScreen(
                        "SM_GoToNextScreen")
                end
                ...
                if UnlockedInput then
                    ...
                    if GAMESTATE:IsSideJoined(self.pn) then
                        ...
                        if type(GroupsAndSongs[TF_WHEEL.CurSong]) == "string" then
                            ...
                            GroupsAndSongs =
                                LoadModule("Group.Sort.lua")(Songs, "")
        ...
        DiffSelectionCommand = function(self)
            ...
            if GAMESTATE:IsSideJoined(PLAYER_2) then
                self:GetChild("Diffs"):GetChild("DiffSelector12"):effectclock(
                    "Beat"):glowshift()
            end
        end,
        ...
        Def.BitmapText {
            ...
            OnCommand = function(self)
                ...
                self:y(10):diffuse(color("#88ff88")):strokecolor(0, 0, 0, 1)
                    :zoom(.25)
            end
        }
    }
