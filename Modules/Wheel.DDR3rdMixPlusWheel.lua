...
local function MoveSelection(self, offset, Songs)
    ...
    self:GetChild("CDSel"):GetChild("CDHolder2"):GetChild("CD"):SetTarget(
        self:GetChild("CDCon"):GetChild("CD" .. CDSwitch):GetChild("Container")
            :GetChild("CDHolder"))
    ...
    local pos = TF_WHEEL.CurSong + (10 * offset)
    ...
    if type(Songs[pos]) ~= "string" then
        ...
        if Songs[pos][1]:HasBanner() then
            if TF_WHEEL.MaskMode then
                self:GetChild("CDCon"):GetChild("CD" .. ChangeOffset):GetChild(
                    "Container"):GetChild("CDHolder"):GetChild("CDPicture")
        ...
        if TF_WHEEL.MaskMode then
            self:GetChild("CDCon"):GetChild("CD" .. ChangeOffset):GetChild(
                "Container"):GetChild("CDHolder"):GetChild("CDPicture"):setsize(
                -120, -120)
        else
    ...
    elseif offset == 0 then
        ...
        for i = 1, 21 do
            ...
            local pos = TF_WHEEL.CurSong + i - 11
            ...
            -- Same as CDOffset, Stay withing limits.
            while CDSliceOffset > 21 do
                CDSliceOffset = CDSliceOffset - 21
            end
                ...
                if Songs[pos][1]:HasBanner() then
                    if TF_WHEEL.MaskMode then
                        self:GetChild("CDCon"):GetChild("CD" .. CDSliceOffset)
                            ...
                            :GetChild("CDPicture"):LoadCachedBanner(
...
local function StartSelection(self, Songs)
    ...
    for i = 1, 21 do
        ...
        if i == CDSwitch then
            if TF_WHEEL.MaskMode then
                self:GetChild("CDCon"):GetChild("CD" .. i):GetChild("Container")
                    :GetChild("CDHolder"):linear(.8):diffusealpha(0)
            else
                self:GetChild("Con"):GetChild("CDSlice" .. i):linear(.8)
                    :diffusealpha(0)
            end
        else
            self:GetChild("CDCon"):GetChild("CD" .. i):GetChild("Container")
                :GetChild("CDHolder"):linear(.8):y(-2560)
        end
    end
end
...
local function MoveDifficulty(self, offset, Songs)
    ...
    if type(Songs[TF_WHEEL.CurSong]) == "string" then
        ...
        for i = 1, DiffCount do
            self:GetChild("Diffs"):GetChild("Feet" .. i):diffusealpha(1):x(25 *
                                                                               (i -
                                                                                   ((DiffCount /
                                                                                       2) +
                                                                                       .5)))
            self:GetChild("Diffs"):GetChild("FeetShadow" .. i):diffusealpha(.5)
                :x(2 + 25 * (i - ((DiffCount / 2) + .5)))
        end

        -- Set the name of the Chart Difficulty.
        self:GetChild("DiffChart"):settext(DiffChartNames[DiffCount]):diffuse(
            DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] +
                1]:GetDifficulty()]]):strokecolor(
            DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] +
                1]:GetDifficulty()]])
...
return function(Style)
        ...
        if TF_WHEEL.MaskMode then
            CDs[#CDs + 1] = Def.ActorFrame {
                ...
                OnCommand = function(self)
                    ...
                    self:rotationz(180 - (((WheelOffset / 21) * (i - 11))) * -1)
                        :y(-80):rotationx(-52):SetFOV(80)

                    -- Offset the wheel before and after the font most CD.
                    if CDSwitch < i then
                        self:addrotationz((WheelOffset / 21) * 2)
                    end
                    if CDSwitch > i then
                        self:addrotationz((-WheelOffset / 21) * 2)
                    end
                ...
                Def.ActorFrame {
                    ...
                    Def.ActorFrame {
                        ...
                        Def.ActorProxy {
                            ...
                            InitCommand = function(self)
                                self:SetTarget(
                                    self:ForParent(5):GetChild("CDBGCon")
    ...
    for i = 0, 1 do
        CDSel[#CDSel + 1] = Def.ActorFrame {
            ...
            Def.ActorProxy {
                Name = "CD",
                InitCommand = function(self)
                    self:SetTarget(self:ForParent(3):GetChild("CDCon"):GetChild(
                                       "CD" .. 11):GetChild("Container")
                                       :GetChild("CDHolder"))
                    if TF_WHEEL.MaskMode then self:zoom(-1) end
                end
            }
        }
    end
    ...
    local ArSel = Def.ActorFrame {
        Def.Sprite {
            ...
            ColourCommand = function(self)
                self:sleep(0.02):diffuse(0, 0, 1, .5):sleep(0.02):diffuse(1, 1,
                                                                          1, .5)
                    :sleep(0.02):diffuse(1, 0, 0, .5)
            end
        }
    }
    ...
    for i = 0, 1 do
        ArSel[#ArSel + 1] = Def.Sprite {
            ...
            OnCommand = function(self)
                self:xy(-10 * i, -10 * i):diffuse(i + .2, i + .2, i + .2, 1)
            end
        }
    end
    ...
    for i = 1, 9 do
        Diff[#Diff + 1] = Def.Sprite {
            ...
            InitCommand = function(self)
                self:zoomx(-.3):zoomy(.3):xy(2 + 25 * (i - 5), 2):diffuse(0, 0,
                                                                          0, 0)
            end
        }
        Diff[#Diff + 1] = Def.Sprite {
    ...
    return Def.ActorFrame {
        ...
        ChangeCDCommand = function(self)
            self:GetChild("CDSel"):GetChild("CDHolder1"):GetChild("CD")
                :SetTarget(self:GetChild("CDCon")
                               :GetChild("CD" .. self.CDSwitch)
                               :GetChild("Container"):GetChild("CDHolder"))
        end,
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                ...
                self:y(-60):zoom(TF_WHEEL.Resize(self:GetWidth(),
                                                 self:GetHeight(),
                                                 (512 / 8) * 5, (160 / 8) * 5))
            end
        },
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                ...
                if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" and
                    GroupsAndSongs[TF_WHEEL.CurSong][1]:HasCDTitle() then
                ...
                self:xy(70, 30):zoom(TF_WHEEL.Resize(self:GetWidth(),
        ...
        Def.BitmapText {
            ...
            Font = "_noto sans 40px",
            OnCommand = function(self)
                self:y(-120):diffuse(.5, .5, 1, 1):strokecolor(0, 0, 1, 1):zoom(
                    .5):zoomy(.4)
            end
        },
        ...
        Def.BitmapText {
            ...
            Font = "_noto sans 40px",
            OnCommand = function(self)
                self:x(-120):diffuse(.5, 1, .5, 1):strokecolor(0, .8, 0, 1)
                    :zoom(.4):zoomx(.5)
            end
        },
        ...
        Def.BitmapText {
            Name = "Difficulty",
            Font = "_noto sans 40px",
            OnCommand = function(self) self:y(-110):zoom(.5) end
        },
        ...
        ArSel .. {
            Name = "Left",
            OnCommand = function(self)
                self:xy(-160, 50):rotationz(25):rotationx(-50):SetFOV(80):zoom(
                    .2):GetChild("Inside"):diffuse(1, 0, 0, .5)
            end
        },
        ...
        ArSel .. {
            ...
            OnCommand = function(self)
                self:xy(160, 50):rotationz(-25):rotationx(-50):SetFOV(80):zoom(
                    .2):zoomx(-.2):GetChild("Inside"):diffuse(1, 0, 0, .5)
            end
        },
        ...
        Def.BitmapText {
            ...
            Font = "_noto sans 40px",
            OnCommand = function(self)
                self:xy(2, 182):zoom(.7):zoomy(.5):diffuse(0, 0, 0, .5)
                    :strokecolor(0, 0, 0, .5)
            end
        },
        ...
        Def.BitmapText {
            Name = "DiffChart",
            Font = "_noto sans 40px",
            OnCommand = function(self) self:y(180):zoom(.7):zoomy(.5) end
        }
    }
end
