...
local function MoveSelection(self, offset, Songs)
    ...
    if offset ~= 0 then
        self:GetChild("Selector"):stoptweening():linear(.05):diffusealpha(.2)
            :linear(.05):diffusealpha(1)
        ...
        for i = 1, 10 do
            ...
            self:GetChild("SongWheel"):GetChild("CD" .. i):linear(.1):addx(
                (offset * -240))
            ...
            if (i == IncOffset and offset == -1) or
                ...
                self:GetChild("SongWheel"):GetChild("CD" .. i):sleep(0):addx(
                    (offset * -240) * -10)

                self:GetChild("SongWheel"):GetChild("CD" .. i)
                    :GetChild("CDText"):settext("")

                if type(Songs[pos]) ~= "string" then
                    if Songs[pos][1]:HasJacket() then
                        self:GetChild("SongWheel"):GetChild("CD" .. i):GetChild(
                            "CDTexture"):Load(Songs[pos][1]:GetJacketPath())
                    elseif Songs[pos][1]:HasBackground() then
                        self:GetChild("SongWheel"):GetChild("CD" .. i):GetChild(
                            "CDTexture"):Load(Songs[pos][1]:GetBackgroundPath())
                    else
                        self:GetChild("SongWheel"):GetChild("CD" .. i):GetChild(
                            "CDTexture"):Load(THEME:GetPathG("", "white.png"))
                        self:GetChild("SongWheel"):GetChild("CD" .. i):GetChild(
                            "CDText"):settext(
                    ...
                    if SONGMAN:GetSongGroupBannerPath(Songs[pos]) ~= "" then
                        ...
                        self:GetChild("SongWheel"):GetChild("CD" .. i):GetChild(
                            "CDTexture"):Load(THEME:GetPathG("", "white.png"))
                        self:GetChild("SongWheel"):GetChild("CD" .. i):GetChild(
                            "CDText"):settext(Songs[pos])
                    end
                end
                self:GetChild("SongWheel"):GetChild("CD" .. i):GetChild(
                    "CDTexture"):zoomto(400, 400)
            end
        end
        ...
        if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
            ...
            if Songs[TF_WHEEL.CurSong][1]:HasBanner() then
                ...
                self:GetChild("BannerInfo"):visible(true):diffuse(
                    math.random() + .5, math.random() + .5, math.random() + .5,
                    1):zoomy(0):linear(.08):zoomy(1)
                self:GetChild("BannerInfo"):GetChild("Title"):settext(ToUpper(
                                                                          Songs[TF_WHEEL.CurSong][1]:GetDisplayMainTitle()))
                self:GetChild("BannerInfo"):GetChild("SubTitle"):settext(
                    ToUpper(Songs[TF_WHEEL.CurSong][1]:GetDisplaySubTitle()))
                self:GetChild("BannerInfo"):GetChild("Artist"):settext(ToUpper(
                                                                           Songs[TF_WHEEL.CurSong][1]:GetDisplayArtist()))
            end

            self:GetChild("Info"):GetChild("BPM"):visible(true):settext(
                "BPM " ..
                    string.format("%.0f",
        ...
        self:GetChild("Banner"):zoom(TF_WHEEL.Resize(
                                         ...
                                         (512 / 10) * 5, (160 / 10) * 5)):zoomy(
            0):linear(.08):zoom(TF_WHEEL.Resize(
                                    self:GetChild("Banner"):GetWidth(),
                                    self:GetChild("Banner"):GetHeight(),
                                    (512 / 10) * 5, (160 / 10) * 5))
    else
        ...
        for i = 1, 10 do
            ...
            if i > 5 then pos = TF_WHEEL.CurSong + i - 10 end

            -- Keep pos withing limits.
            while pos > #Songs do pos = pos - #Songs end
            while pos < 1 do pos = #Songs + pos end

            self:GetChild("SongWheel"):GetChild("CD" .. off):GetChild("CDText")
                :settext("")

            if type(Songs[pos]) ~= "string" then
                if Songs[pos][1]:HasJacket() then
                    self:GetChild("SongWheel"):GetChild("CD" .. off):GetChild(
                        "CDTexture"):Load(Songs[pos][1]:GetJacketPath())
                elseif Songs[pos][1]:HasBackground() then
                    self:GetChild("SongWheel"):GetChild("CD" .. off):GetChild(
                        "CDTexture"):Load(Songs[pos][1]:GetBackgroundPath())
                    ...
                    self:GetChild("SongWheel"):GetChild("CD" .. off):GetChild(
                        "CDTexture"):Load(THEME:GetPathG("", "white.png"))
                    self:GetChild("SongWheel"):GetChild("CD" .. off):GetChild(
                        "CDText"):settext(Songs[pos][1]:GetDisplayMainTitle())
                end
            else
                if SONGMAN:GetSongGroupBannerPath(Songs[pos]) ~= "" then
                    self:GetChild("SongWheel"):GetChild("CD" .. off):GetChild(
...
local function MoveDifficulty(self, offset, Songs)
    ...
    if type(Songs[TF_WHEEL.CurSong]) == "string" then
        ...
        self:GetChild("DiffDisplay"):GetChild("BG"):diffuse(
            DiffColors[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] +
                1]:GetDifficulty()]])
        self:GetChild("DiffDisplay"):GetChild("Text"):settext(
            DiffNames[TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] +
                1]:GetDifficulty()]])
        self:GetChild("Info"):GetChild("ChartArtist"):visible(true):settext(
            Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] + 1]:GetAuthorCredit())
    end
end
...
return function(Style)
    ...
    for i = 1, 10 do
        ...
        SongWheel[#SongWheel + 1] = Def.ActorFrame {
            ...
            Def.Sprite {Texture = THEME:GetPathG("", "PPP/CD.png")},
            Def.Sprite {
                ...
                OnCommand = function(self)
                    if type(GroupsAndSongs[pos]) ~= "string" then
                        ...
                        if SONGMAN:GetSongGroupBannerPath(GroupsAndSongs[pos]) ~=
                            "" then
                            self:Load(SONGMAN:GetSongGroupBannerPath(
                                          GroupsAndSongs[pos]))
                        end
                    end
            ...
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    if type(GroupsAndSongs[pos]) ~= "string" then
                        if not GroupsAndSongs[pos][1]:HasJacket() and
                            not GroupsAndSongs[pos][1]:HasBackground() then
                            self:settext(
    ...
    return Def.ActorFrame {
        ...
        MenuLeftCommand = function(self)
            ...
            MoveDifficulty(self, 0, GroupsAndSongs)
            self:GetChild("Select"):GetChild("LeftCon"):GetChild("LeftArrow")
                :diffuse(1, 1, 0, .6):sleep(.08):diffusealpha(0)
        end,
        ...
        MenuRightCommand = function(self)
            MoveSelection(self, 1, GroupsAndSongs)
            MoveDifficulty(self, 0, GroupsAndSongs)
            self:GetChild("Select"):GetChild("RightCon"):GetChild("RightArrow")
                :diffuse(1, 1, 0, .6):sleep(.08):diffusealpha(0)
        end,
        ...
        StartCommand = function(self)
            self:GetChild("Select"):GetChild("Start"):GetChild("StartButton")
                :diffuse(1, 0, 0, .6):sleep(.08):diffusealpha(0)
        ...
        SongWheel .. {OnCommand = function(self) self:z(-200) end},
        ...
        Def.ActorFrame {
            OnCommand = function(self) self:xy(-160, 36) end,
            Def.Quad {
                OnCommand = function(self)
            ...
            Def.BitmapText {
                ...
                Font = "_noto sans 40px",
                OnCommand = function(self)
                    self:zoom(.45):xy(70, 74):diffuse(.5, .5, .5, 1)
                        :strokecolor(.5, .5, .5, 1):zoomy(.65)
                end
            }
        },
        ...
        Def.ActorFrameTexture {
            ...
            InitCommand = function(self)
                self:SetTextureName("DiffAFT"):SetWidth(360):SetHeight(60)
                    :EnableAlphaBuffer(true):Create():Draw()
            end,
            Def.Quad {
                OnCommand = function(self)
        ...
        Def.ActorFrame {
            Name = "DiffDisplay",
            OnCommand = function(self) self:xy(-58, -130):zoomy(.85) end,
            Def.Quad {
                Name = "BG",
                OnCommand = function(self)
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        self:zoomto(148, 38):diffuse(
                            DiffColors[TF_WHEEL.DiffTab[GroupsAndSongs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] +
                                1]:GetDifficulty()]])
                    else
            ...
            Def.Sprite {
                OnCommand = function(self) self:zoom(.4) end,
            ...
            Def.BitmapText {
                ...
                Font = "_noto sans 40px",
                OnCommand = function(self)
                    self:zoom(.6):diffuse(1, 1, 1, 1):maxwidth(320):strokecolor(
                        1, 1, 1, 1)
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        self:settext(
                            DiffNames[TF_WHEEL.DiffTab[GroupsAndSongs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] +
                                1]:GetDifficulty()]])
                    else
        ...
        Def.Sprite {
            ...
            OnCommand = function(self)
                ...
                self:xy(-160, -40):zoom(TF_WHEEL.Resize(self:GetWidth(),
                                                        self:GetHeight(),
                                                        (512 / 10) * 5,
                                                        (160 / 10) * 5))
            end
        ...
        Def.ActorFrame {
            Name = "BannerInfo",
            OnCommand = function(self)
                self:x(-160):y(-30):diffuse(math.random() + .5,
                                            math.random() + .5,
                                            math.random() + .5, 1)
                if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
            ...
            Def.Quad {OnCommand = function(self) self:zoomto(320, 2) end},
            Def.BitmapText {
                Name = "Title",
                Font = "_noto sans 40px",
                OnCommand = function(self)
                    self:diffuse(1, 1, 1, 1):y(-30):maxwidth(320)
                    -- Check if we are on song
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        self:settext(ToUpper(
            ...
            Def.BitmapText {
                ...
                Font = "_noto sans 40px",
                OnCommand = function(self)
                    self:zoom(.2):diffuse(1, 1, 1, 1):y(-10):maxwidth(1600)

                    -- Check if we are on song
                    if type(GroupsAndSongs[TF_WHEEL.CurSong]) ~= "string" then
                        self:settext(ToUpper(
                                         GroupsAndSongs[TF_WHEEL.CurSong][1]:GetDisplaySubTitle()))
                    end
                end
            },
            Def.BitmapText {
                ...
                Font = "_noto sans 40px",
                OnCommand = function(self)
                    self:zoom(.8):diffuse(1, 1, 1, 1):y(20):maxwidth(400)
        ...
        Def.ActorFrame {
            ...
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    self:zoom(.38):zoomx(.8):diffuse(1, 1, 1, .2):MaskSource()
                end
            },
            Def.BitmapText {
                Font = "_noto sans 40px",
                Text = "Music Title", -- Thanks subo and paraph
                OnCommand = function(self)
                    self:zoom(.4):zoomx(.8):diffuse(1, 1, 1, 1):strokecolor(.8,
                                                                            .6,
                                                                            1, 1)
                        :MaskDest()
                end
        ...
        Def.ActorFrame {
            ...
            Def.BitmapText {
                ...
                OnCommand = function(self)
                    self:zoom(.38):zoomx(.5):diffuse(1, 1, 1, .2):MaskSource()
                end
            },
            Def.BitmapText {
                Font = "_noto sans 40px",
                Text = "Dance Level", -- Thanks subo and paraph
                OnCommand = function(self)
                    self:zoom(.4):zoomx(.5):diffuse(1, 1, 1, 1):strokecolor(.8,
                                                                            .6,
                                                                            1, 1)
                        :MaskDest()
                end
        ...
        Def.ActorFrame {
            ...
            Def.Quad {
                OnCommand = function(self)
                    self:zoomto(154, 26):diffuse(.8, .6, 1, .5):x(-80)
                end
            },
            Def.Quad {
                OnCommand = function(self)
            ...
            Def.BitmapText {
                ...
                Font = "_noto sans 40px",
                OnCommand = function(self)
                    self:zoom(.5):diffuse(1, 1, 1, 1):halign(0):x(-150)
                        :maxwidth(260)
            ...
            Def.BitmapText {
                ...
                Font = "_noto sans 40px",
                OnCommand = function(self)
                    self:zoom(.5):diffuse(1, 1, 1, 1):x(80):maxwidth(260)
        ...
        Def.BitmapText {
            ...
            Text = ToEnumShortString(GAMESTATE:GetCurrentStage()):upper() ..
                " STAGE",
            OnCommand = function(self)
                self:zoom(.7):diffuse(1, 1, 1, 1):strokecolor(color("#ffc0cb"))
                    :halign(1):xy(-150, -126)
            end
        },
        ...
        Def.ActorFrame {
            ...
            Def.ActorFrame {
                ...
                Def.ActorFrame {
                    Name = "Left",
                    OnCommand = function(self) self:x(-70) end,
                    Def.Sprite {
                    ...
                    Def.Sprite {
                        Texture = THEME:GetPathG("", "Triangle.png"),
                        OnCommand = function(self)
                            self:zoom(.04):rotationz(-90):diffuse(0, 0, 0, 1):x(
                                1)
                        end
                    },
                    Def.Sprite {
                        Texture = THEME:GetPathG("", "Triangle.png"),
                        OnCommand = function(self)
                            self:zoom(.03):rotationz(-90):diffuse(.6, .4, 0, 1)
                                :x(2)
                        end
                    },
                    Def.Sprite {
                        Texture = THEME:GetPathG("", "Triangle.png"),
                        OnCommand = function(self)
                            self:zoom(.03):rotationz(-90):diffuse(1, 1, 0, .8)
                                :x(2):fadeleft(.5):faderight(.3)
                        end
                    }
                },
                Def.Sprite {
            ...
            Def.ActorFrame {
                ...
                Def.ActorProxy {
                    OnCommand = function(self)
                        self:SetTarget(self:GetParent():GetParent():GetChild(
                                           "LeftCon"):GetChild("Left")):x(-114)
                            :zoomx(-1)
                    end
                },
                Def.Sprite {
            ...
            Def.ActorFrame {
                Name = "Start",
                OnCommand = function(self) self:x(35) end,
                Def.Quad {
                    OnCommand = function(self)
