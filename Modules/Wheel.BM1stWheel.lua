...
local function MoveSelection(self, offset, Songs)
    ...
    for i = 1, 3 do
        self:GetChild("LPCon"):GetChild("LP" .. i):linear(.1):addrotationz(
            ((360 / 3) * offset) * -1)
    end
    ...
    if type(Songs[pos]) ~= "string" then
        ...
        if Songs[pos][1]:HasBanner() then
            self:GetChild("LPCon"):GetChild("LP" .. ChangeOffset):GetChild(
        ...
        self:GetChild("LPCon"):GetChild("LP" .. ChangeOffset):GetChild(
            "Container"):GetChild("LPPicture"):setsize(-180, -180)
    ...
    elseif offset == 0 then
        ...
        for i = 1, 3 do
            ...
            -- Same as LPOffset, Stay withing limits.
            while LPSliceOffset > 3 do
                LPSliceOffset = LPSliceOffset - 3
            end
            while LPSliceOffset < 1 do
                LPSliceOffset = 1 + LPSliceOffset
            end
            ...
            if type(Songs[pos]) ~= "string" then
                -- We check if the song has a banner, We use this for the LPs, If there is no banner, use fallback banner.png
                if Songs[pos][1]:HasBanner() then
                    self:GetChild("LPCon"):GetChild("LP" .. LPSliceOffset)
                        :GetChild("Container"):GetChild("LPPicture")
            ...
            self:GetChild("LPCon"):GetChild("LP" .. LPSliceOffset):GetChild(
                "Container"):GetChild("LPPicture"):setsize(-180, -180)
        end
    else
        -- Its a song group, Set it to group banner, If it doesnt have a banner, Use fallback banner.png
    ...
    if type(Songs[TF_WHEEL.CurSong]) ~= "string" then
        ...
        if not Songs[TF_WHEEL.CurSong][1]:HasBanner() then
            ...
            self:GetChild("BannerAFT"):GetChild("BannerText"):settext("")
                :maxwidth(280):maxheight(160):Regen()
        end
        -- Its a group.
    else
        -- Set banner.
...
return function(Style)
    ...
    return Def.ActorFrame {
        ...
        Def.ActorFrameTexture {
            Name = "BannerAFT",
            InitCommand = function(self)
                self:SetTextureName("BannerAFT"):SetWidth(280):SetHeight(160)
        ...
        Def.Sprite {
            Texture = "BannerAFT",
            OnCommand = function(self)
                self:y(-50):croptop(.32):diffuse(0, 0, 0, .7):diffusebottomedge(
                    1, .5, 0, 1)
            end
        },
        ...
        Def.ActorFrameTexture {
            InitCommand = function(self)
                self:SetTextureName("SSAFT"):SetWidth(SCREEN_WIDTH):SetHeight(
                    SCREEN_HEIGHT):EnableAlphaBuffer(true):Create():Draw()
            end,
            Def.Text {
                ...
                StrokeSize = 2,
                OnCommand = function(self)
                    self:settext("SONG SELECT"):Regen():CenterX():y(80)

                    self:MainActor():diffuse(.3, .3, .3, 1)

                    self:StrokeActor():diffuse(1, 1, 1, .8)
                end
            }
        },
        Def.Sprite {
            Texture = "SSAFT",
            OnCommand = function(self) self:texcoordvelocity(.1, 0) end
        },
