...
local function MoveSelection(self,offset,Songs)
	...
	if offset ~= 0 then
		...
		for i = 1,9 do			
			...
			if (i == IncOffset and offset == -1) or (i == DecOffset and offset == 1) then
				...
				if type(Songs[pos]) ~= "string" then
					...
					if SONGMAN:GetSongGroupBannerPath(Songs[pos]) ~= "" then
						self:GetChild("Wheel"):GetChild("Container"..i):GetChild("Jacket"):visible(1):Load(SONGMAN:GetSongGroupBannerPath(Songs[pos]))
					else
						self:GetChild("Wheel"):GetChild("Container"..i):GetChild("Jacket"):visible(0)
					end
				end
			...
			if i == XOffset then
				...
				self:GetChild("Wheel"):GetChild("Container"..i):GetChild("JacketShadow"):decelerate(.1):zoomto(135,135/2):y(-20+135/2)
		...
		if type(Songs[CurSong]) ~= "string" then
			self:GetChild("CurSongTitle"):settext(Songs[CurSong][1]:GetDisplayMainTitle()):diffusealpha(0):y(0):decelerate(.1):y(30):diffusealpha(1)
		else
	...
	-- We are on an offset of 0.
		...
		for i = 1,9 do
			...
			if type(Songs[pos]) ~= "string" then
				...
				if SONGMAN:GetSongGroupBannerPath(Songs[pos]) ~= "" then
					self:GetChild("Wheel"):GetChild("Container"..off):GetChild("Jacket"):visible(1):Load(SONGMAN:GetSongGroupBannerPath(Songs[pos]))
				else
					self:GetChild("Wheel"):GetChild("Container"..off):GetChild("Jacket"):visible(0)
				end
			end
	...
	-- Check if we are on song.
	if type(Songs[CurSong]) ~= "string" then
		self:GetChild("Data"):GetChild("Artist"):settext("Artist : "..Songs[CurSong][1]:GetDisplayArtist().."  Producer : "..Songs[CurSong][2]:GetAuthorCredit())
		if Songs[CurSong][1]:GetDisplaySubTitle() == "" then
			self:GetChild("Data"):GetChild("SubTitle"):settext(TF_WHEEL.FormatAddNewline(Songs[CurSong][1]:GetTags(),60)):stopeffect():y(-5)
		else
			self:GetChild("Data"):GetChild("SubTitle"):settext(TF_WHEEL.FormatAddNewline(Songs[CurSong][1]:GetDisplaySubTitle(),60)):stopeffect():y(-5)
		end
		...
		if self:GetChild("Data"):GetChild("SubTitle"):GetHeight() > 140 then
			self:GetChild("Data"):GetChild("SubTitle"):y(-self:GetChild("Data"):GetChild("SubTitle"):GetHeight()*.125):bob()
				...
				:effecttiming(self:GetChild("Data"):GetChild("SubTitle"):GetHeight()/50,0,self:GetChild("Data"):GetChild("SubTitle"):GetHeight()/50,0,0)
		end
...
return function(Style)
	...
	return Def.ActorFrame{
		...
		Def.BitmapText{
			...
			InitCommand=function(self)
				self:zoomy(1.4):zoomx(1.8):xy(-SCREEN_CENTER_X+47,-SCREEN_CENTER_Y):rotationz(90):diffuse(0,0,0,0):decelerate(.6):diffusealpha(.2):y(-40)
			end
		},
		Def.Quad{
			Name="Line",
			InitCommand=function(self)
				self:zoomto(2,2):xy(-SCREEN_CENTER_X+60,-SCREEN_CENTER_Y-2):diffuse(0,0,0,1):valign(0):decelerate(1):zoomto(2,SCREEN_HEIGHT+4)
			end
		},
		Def.ActorProxy{
			InitCommand=function(self)
		...
		Def.ActorFrame{
			...
			Def.BitmapText{
				...
				BPMChangeCommand=function(self)
					if type(GroupsAndSongs[CurSong]) ~= "string" then
						self:settext("Artist : "..GroupsAndSongs[CurSong][1]:GetDisplayArtist().."  Producer : "..GroupsAndSongs[CurSong][2]:GetAuthorCredit())
					else
						self:settext("Artist :")
					end
				end,
