...
local function StartSelection(self, Songs)
	...
	for i = 1, 37 do
		...
		if i == CDSwitch then
			self:GetChild("CDCon"):GetChild("CD" .. i):GetChild("Container"):accelerate(1.5):addrotationz(-360):linear(.4)
				:addy(-
					1280)
		else
...
local function MoveDifficulty(self, offset, Songs)
	...
	if type(Songs[TF_WHEEL.CurSong]) == "string" then
		...
		for i = 1, 9 do
			...
			TF_WHEEL.DiffTab[Songs[TF_WHEEL.CurSong][TF_WHEEL.DiffPos[DiffPlayer] + 1]:GetDifficulty()]]):diffusealpha(0)
		end
				...
				Def.ActorFrame {
					...
					Def.ActorProxy {
						...
						InitCommand = function(self)
							self:SetTarget(self:ForParent(4):GetChild("CDBGCon"):GetChild("CDBG")):zoom(.23)
						end
					},
					Def.Sprite {
						Name = "CDMask",
			...
			CDslice[#CDslice + 1] = Def.Sprite {
				...
				OnCommand = function(self)
					...
					self:setsize(512, 160):SetCustomPosCoords(self:GetWidth() / 2 - 23, 0, self:GetWidth() / 2 - 9, -80,
						-self:GetWidth() / 2 + 9, -80, -self:GetWidth() / 2 + 23, 0):zoom(.4):y(-20):visible(false)
				end
			}
	...
	return Def.ActorFrame {
		...
		Def.BitmapText {
			...
			OnCommand = function(self)
				...
				self:y(-80):diffuse(1, 1, 0, 1):strokecolor(0, 0, 1, 1):zoom(.5):maxwidth(600)
			end
		},
		...
		TriSel .. {
			...
			ColourCommand = function(self)
				self:sleep(0.02):diffuse(0, 0, 1, 1):sleep(0.02):diffuse(1, 1, 1, 1):sleep(0.02):
					diffuse(1, 0, 0, 1)
			end
		},
